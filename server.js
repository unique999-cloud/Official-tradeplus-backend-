...
// Free plan: 8 calls/min. Batch = 1 call for all pairs
// ============================================================
const batchPriceCache = { data:{}, ts:0 };

async function fetchBatchPrices(){
  const now = Date.now();
  if(now - batchPriceCache.ts < 25000) return batchPriceCache.data; // 25s cache
  if(!TWELVE_KEY) return {};
  try{
    const symbols = 'EUR/USD,GBP/USD,USD/JPY,XAU/USD';
    const r = await fetch(`https://api.twelvedata.com/price?symbol=${encodeURIComponent(symbols)}&apikey=${TWELVE_KEY}`,{timeout:4000});
    const d = await r.json();
    // Response: {"EUR/USD":{"price":"1.16428"},"GBP/USD":{"price":"1.34806"},...}
    const prices = {};
    for(const [sym,val] of Object.entries(d)){
      const p = parseFloat(val?.price);
      if(p>0&&!isNaN(p)){
        prices[sym] = p;
        console.log(`✅ Batch price ${sym}: ${p}`);
      }
    }
...
async function getDXY() {
  ...
  const sources=[
    // Twelve Data DXY
    async()=>{ if(!TWELVE_KEY) return null; const r=await fetch(`https://api.twelvedata.com/price?symbol=DXY&apikey=${TWELVE_KEY}`,{timeout:3000}); const d=await r.json(); const v=parseFloat(d.price); return v>80&&v<120?v:null; },
    // Stooq DXY
    ...
    // Stooq alternate symbol
    async()=>{ const r=await fetch('https://stooq.com/q/l/?s=usdx.fxb&f=sd2t2ohlcv&h&e=csv',{timeout:3000}); const t=await r.text(); const p=t.trim().split('\n')[1]?.split(','); const v=parseFloat(p?.[6]); return v>80&&v<120?v:null; },
  ];
  for(const src of sources){
    try{
      const v=await src();
  ...
  // Return last real cached value or approximate
