...
async function getDXY() {
  ...
  try{
    const res=await fetch('https://stooq.com/q/l/?s=dxy&f=sd2t2ohlcv&h&e=csv',{timeout:3000});
    const text=await res.text();
    ...
    if(cachedSignals.length>0){
      ...
      for(const pair of missingSyns){
        ...
        cachedSignals.push({
          ...
          reasons:[{icon:'⏳',text:'Loading real data... prices updating shortly'}],
          warnings:[],candles:[],updatedAt:new Date().toISOString()
        });
      }
