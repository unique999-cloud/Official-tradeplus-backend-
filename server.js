...
async function getStooqPrice(stooqSym) {
  ...
  if(stooqSym.includes('xau')) {
    const sources=[
      ...
      async()=>{ const r=await fetch('https://api.metals.live/v1/spot/gold',{timeout:3000}); const t=await r.text(); const d=JSON.parse(t); const v=Array.isArray(d)?d.find(x=>x.gold)?.gold:d.price; return v&&v>1000&&v<6000?parseFloat(v):null; },
      async()=>{ const r=await fetch('https://open.er-api.com/v6/latest/XAU',{timeout:3000}); const d=await r.json(); const v=d?.rates?.USD; return v&&v>1000&&v<6000?parseFloat(v):null; },
    ];
