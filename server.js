...
async function getDXY() {
  ...
  for(const src of sources){
    try{ const v=await src(); if(v){ cache.dxy={value:v,ts:now}; return v; } }catch(e){continue;}
  }
  // Return last cached value or approximate (DXY currently around 100)
  return cache.dxy.value||100.0;
}
...
// ============================================================
// ============================================================
// RISK CALCULATOR — Based on ₦2000 balance, 1:2000 leverage
// ============================================================
function calcRisk(price,sl,symbol,balance=2000,riskPct=1){
  const isGold = symbol==='XAU/USD';
  const NGN_USD_RATE = 1620; // NGN per USD
  const NGN_BALANCE = balance; // ₦2000
  const USD_BALANCE = NGN_BALANCE / NGN_USD_RATE; // ~$1.23

  const riskPctActual = isGold ? 0.5 : riskPct; // Gold uses 0.5%
  const riskNGN = NGN_BALANCE * (riskPctActual/100); // ₦10 gold, ₦20 others
  const riskUSD = riskNGN / NGN_USD_RATE; // ~$0.006-0.012
  ...
  // With ₦2000 at 1:2000 leverage, max safe lot is 0.01 (micro lot)
  const maxSafeLot = isGold ? 0.01 : 0.01;
  ...
  return{
    ...
    isGold,
    note:`₦${NGN_BALANCE} balance | 1% risk = ₦${riskNGN.toFixed(0)} | Max lot: ${maxSafeLot}`,
    goldNote: isGold?`Gold: ₦${riskNGN.toFixed(0)} max risk ($${riskUSD.toFixed(4)}). Use 0.01 lot only.`:null,
  };
}
