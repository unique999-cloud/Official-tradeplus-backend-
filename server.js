...
async function getSignalForPair(pair){
  ...
  const batchPrice = freshBatch[pair.symbol]; // e.g. "EUR/USD" → 1.1643
  ...
  if(cache.signal[pair.symbol]&&(now-cache.signal[pair.symbol].ts)<TTL.H4){
    // Always update price from batch or individual source
    ...
    if(livePrice){
      ...
      // Refresh signal every 2 minutes even if cached
