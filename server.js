...
const TTL = {
  ...
  PRICE:30*1000, PRICE_GOLD:15*1000, NEWS:5*60*1000, DXY:5*60*1000,
};
...
// ============================================================
// TWELVE DATA BATCH PRICE - all 4 pairs in ONE API call
// ============================================================
const batchPriceCache = { data:{}, ts:0 };

async function fetchBatchPrices(){
  const now = Date.now();
  if(now - batchPriceCache.ts < 15000) return batchPriceCache.data; // 15s cache
  if(!TWELVE_KEY) return {};
  try{
    const symbols = encodeURIComponent('EUR/USD,GBP/USD,USD/JPY,XAU/USD');
    const r = await fetch(`https://api.twelvedata.com/price?symbol=${symbols}&apikey=${TWELVE_KEY}`,{timeout:4000});
    const text = await r.text();
    const d = JSON.parse(text);

    // Handle both batch response and error
    if(d.code||d.status==='error'){
      console.log('Batch price error:', d.message||d.code);
      return batchPriceCache.data;
    }

    const prices = {};
    // Response can be {EUR/USD:{price:x}} or {price:x} if single symbol
    const entries = d['EUR/USD']?d:{'EUR/USD':d};
    for(const [sym,val] of Object.entries(d)){
      if(typeof val === 'object' && val?.price){
        const p = parseFloat(val.price);
        if(p>0&&!isNaN(p)) prices[sym]=p;
      }
    }

    if(Object.keys(prices).length >= 2){
      batchPriceCache.data = prices;
      batchPriceCache.ts = now;
      console.log(`✅ Batch prices: ${Object.entries(prices).map(([k,v])=>`${k}:${v}`).join(', ')}`);
    }
    return batchPriceCache.data;
  }catch(e){
...
// Get NGN/USD live rate
async function getNGNRate(){
  try{
    const r=await fetch('https://open.er-api.com/v6/latest/USD',{timeout:3000});
    const d=await r.json();
    const rate=d?.rates?.NGN;
    return rate&&rate>1000?parseFloat(rate):1372;
  }catch(e){ return 1372; }
}

let cachedNGNRate = 1372;
// Update NGN rate every hour
setInterval(async()=>{
  cachedNGNRate = await getNGNRate();
  console.log(`NGN/USD rate updated: ${cachedNGNRate}`);
}, 60*60*1000);

async function getTwelveQuote(symbol) {
  // Try batch first
  const batch = await fetchBatchPrices();
  if(batch[symbol]) return batch[symbol];
  // Individual fallback
  if(!TWELVE_KEY) return null;
  try{
    const r=await fetch(`https://api.twelvedata.com/price?symbol=${encodeURIComponent(symbol)}&apikey=${TWELVE_KEY}`,{timeout:3000});
    const d=await r.json();
...
async function getStooqPrice(stooqSym) {
  ...
  if(stooqSym.includes('xau')) {
    const sources=[
      async()=>{ const v=await getTwelveQuote('XAU/USD'); return v>1000&&v<8000?v:null; },
      async()=>{ const r=await fetch('https://stooq.com/q/l/?s=xauusd.cf&f=sd2t2ohlcv&h&e=csv',{timeout:3000}); const t=await r.text(); const p=t.trim().split('\n')[1]?.split(','); const v=parseFloat(p?.[6]); return v>1000&&v<8000?v:null; },
      async()=>{ const r=await fetch('https://stooq.com/q/l/?s=xauusd&f=sd2t2ohlcv&h&e=csv',{timeout:3000}); const t=await r.text(); const p=t.trim().split('\n')[1]?.split(','); const v=parseFloat(p?.[6]); return v>1000&&v<8000?v:null; },
      async()=>{ const r=await fetch('https://api.metals.live/v1/spot/gold',{timeout:3000}); const t=await r.text(); const d=JSON.parse(t); const v=Array.isArray(d)?d.find(x=>x.gold)?.gold:d.price; return v&&v>1000&&v<8000?parseFloat(v):null; },
      async()=>{ const r=await fetch('https://open.er-api.com/v6/latest/XAU',{timeout:3000}); const d=await r.json(); const v=d?.rates?.USD; return v&&v>1000&&v<8000?parseFloat(v):null; },
    ];
  ...
  // Forex pairs - Twelve Data quote FIRST (real-time), then fallbacks
  ...
  const twelveSymbol=symbol==='USDJPY'?'USD/JPY':symbol==='EURUSD'?'EUR/USD':symbol==='GBPUSD'?'GBP/USD':'EUR/USD';

  const sources=[
    // Twelve Data real-time quote (most accurate, matches MT5)
    async()=>{ const v=await getTwelveQuote(twelveSymbol); return v>0?v:null; },
    // Stooq CSV
    async()=>{ const r=await fetch(`https://stooq.com/q/l/?s=${stooqSym}&f=sd2t2ohlcv&h&e=csv`,{timeout:3000}); const t=await r.text(); if(t.includes('N/D')||t.includes('apikey')) return null; const p=t.trim().split('\n')[1]?.split(','); const v=parseFloat(p?.[6]); return v>0?v:null; },
    // Frankfurter API
    async()=>{ if(quote!=='USD'&&base!=='USD') return null; const from=base==='USD'?quote:base; const to=base==='USD'?base:quote; const r=await fetch(`https://api.frankfurter.app/latest?from=${from}&to=${to}`,{timeout:3000}); const d=await r.json(); const v=d?.rates?.[to]; if(!v) return null; return base==='USD'?parseFloat((1/v).toFixed(5)):parseFloat(v); },
    // ExchangeRate API
    async()=>{ const r=await fetch(`https://open.er-api.com/v6/latest/${base}`,{timeout:3000}); const d=await r.json(); const v=d?.rates?.[quote]; return v?parseFloat(v):null; },
  ];
...
async function getStooqDaily(stooqSym) {
  ...
  try{
    const res=await fetch(`https://stooq.com/q/d/l/?s=${dailySym}&i=d`,{timeout:3000});
...
async function getDXY() {
  const now=Date.now();
  if(cache.dxy.value&&cache.dxy.value!==100.0&&(now-cache.dxy.ts)<TTL.DXY) return cache.dxy.value;
  const sources=[
    // Yahoo Finance DXY
    async()=>{
      const r=await fetch('https://query1.finance.yahoo.com/v8/finance/chart/DX-Y.NYB?interval=1m&range=1d',{timeout:4000,headers:{'User-Agent':'Mozilla/5.0'}});
      const d=await r.json();
      const v=d?.chart?.result?.[0]?.meta?.regularMarketPrice;
      return v&&v>80&&v<120?parseFloat(v.toFixed(2)):null;
    },
    // Twelve Data DXY
    async()=>{
      if(!TWELVE_KEY) return null;
      const r=await fetch(`https://api.twelvedata.com/price?symbol=DXY&apikey=${TWELVE_KEY}`,{timeout:3000});
      const d=await r.json();
      const v=parseFloat(d.price);
      return v>80&&v<120?v:null;
    },
    // Stooq DXY
    async()=>{ const r=await fetch('https://stooq.com/q/l/?s=dxy&f=sd2t2ohlcv&h&e=csv',{timeout:3000}); const t=await r.text(); const p=t.trim().split('\n')[1]?.split(','); const v=parseFloat(p?.[6]); return v>80&&v<120?v:null; },
  ];
  for(const src of sources){
    try{
      const v=await src();
      if(v&&v!==100.0){
        cache.dxy={value:parseFloat(v.toFixed(2)),ts:now};
        console.log(`✅ DXY: ${v}`);
        return cache.dxy.value;
      }
    }catch(e){continue;}
  }
  return cache.dxy.value&&cache.dxy.value!==100.0?cache.dxy.value:null;
}
...
function calcADX(c,p=14){
  ...
  // Validate candle quality first - flat/repeated candles give wrong ADX
  const uniqueCloses = new Set(c.slice(-10).map(x=>x.close.toFixed(5))).size;
  if(uniqueCloses < 3) return null; // candles too flat/repeated = unreliable

  // Cap ADX at 60 - forex ADX rarely exceeds 60 in real markets
  const cappedDX = Math.min(dx, 60);
  return{adx:parseFloat(cappedDX.toFixed(2)),trending:cappedDX>25,strongTrend:cappedDX>40,bullish:diP>diM};
}
...
async function getTreasuryYield(){
  try{
    ...
    const res=await fetch('https://stooq.com/q/l/?s=10usy.b&f=sd2t2ohlcv&h&e=csv',{timeout:3000});
    const text=await res.text();
...
// ============================================================
function calcRisk(price,sl,symbol,balance=2000,riskPct=1){
  const isGold = symbol==='XAU/USD';
  const NGN_BALANCE = balance;
  const NGN_USD_RATE = cachedNGNRate || 1372; // Live from ExchangeRate API
  const USD_BALANCE = NGN_BALANCE / NGN_USD_RATE;

  const riskPctActual = isGold ? 0.5 : riskPct;
  const riskNGN = NGN_BALANCE * (riskPctActual/100);
  const riskUSD = riskNGN / NGN_USD_RATE;
  ...
  const pipUSDPerMicroLot = isGold?0.01:symbol==='USD/JPY'?0.00093:0.001;
  const lots = slPips>0 ? riskUSD/(slPips*(pipUSDPerMicroLot/0.01)) : 0.01;
  const maxSafeLot = 0.01;
  const safeLot = Math.min(Math.max(parseFloat(lots.toFixed(2)),0.01),maxSafeLot);

  return{
    riskAmount:riskNGN.toFixed(2),
    riskAmountNGN:`₦${riskNGN.toFixed(2)}`,
    riskAmountUSD:`$${riskUSD.toFixed(4)}`,
    slPips:slPips.toFixed(1),
    slDistance:slDistance.toFixed(isGold?2:5),
    suggestedLot:safeLot,
    ngnRate:NGN_USD_RATE.toFixed(0),
    isGold,
    goldNote: isGold?`Gold: ₦${riskNGN.toFixed(0)} max risk. Use 0.01 lot only.`:null,
  };
}
...
function deepSignalEngine(ind,mtfBias,dxy,symbol){
  ...
  // Block EUR/USD and GBP/USD during Asian session - unreliable signals
  const currentSession=getSession();
  if(['EUR/USD','GBP/USD'].includes(symbol)&&currentSession.name==='ASIAN'){
    return{signal:'WAIT',phase:'ANALYSING',confidence:0,bull:0,bear:0,
      factors:['😴 Asian session - EUR/USD and GBP/USD signals unreliable. Wait for London open 8AM WAT.'],warnings:[]};
  }

  // Block when ALL timeframes are neutral - no clear direction
  const allNeutral = mtfBias.monthly==='NEUTRAL'&&mtfBias.weekly==='NEUTRAL'&&
                     mtfBias.daily==='NEUTRAL'&&mtfBias.h4==='NEUTRAL';
  if(allNeutral){
    return{signal:'WAIT',phase:'ANALYSING',confidence:0,bull:0,bear:0,
      factors:['↔️ All timeframes NEUTRAL - no clear market direction. Waiting for trend to develop.'],warnings:[]};
  }
  ...
  if(bull>bear&&bull>=minScore&&conf>=minConf){
    ...
    // FORMING - must also respect trend direction
    if(bull>bear){
      phase=hardTrendBlock(mtfBias,'BUY')?'BLOCKED_BY_TREND':'BUY_FORMING';
    }else{
      phase=hardTrendBlock(mtfBias,'SELL')?'BLOCKED_BY_TREND':'SELL_FORMING';
    }
  }
...
async function getSignalForPair(pair){
  ...
  // Always try to get fresh price - use batch first, then individual sources
  const freshBatch = await fetchBatchPrices();
  const batchPrice = freshBatch[pair.symbol];

  // Update live price if cached signal exists (under 4 hours)
  if(cache.signal[pair.symbol]&&(now-cache.signal[pair.symbol].ts)<TTL.H4){
    let livePrice = batchPrice || await getStooqPrice(pair.stooq);
    if(livePrice){
      const d=cache.signal[pair.symbol].data;
      d.price=livePrice.toFixed(pair.dp);
      d.entry=livePrice.toFixed(pair.dp);
      // ALWAYS recalculate session - never use cached session
      const currentSession=getSession();
      d.session=currentSession.name;
      d.killzone=currentSession.killzone;
      d.blackout=isBlackout();
      d.newsComingSoon=isNewsWithin4H();
      d.marketClosingSoon=isMarketClosingSoon();
      // Refresh signal every 2 minutes
      if((now-cache.signal[pair.symbol].ts) < 2*60*1000){
    ...
    }else if((now-cache.signal[pair.symbol].ts) < 2*60*1000){
      return cache.signal[pair.symbol].data;
    }
  }

  const [h4,daily,dxy]=await Promise.all([
    getTwelveCandles(pair.twelve,'4h',100,'h4',TTL.H4),
    getStooqDaily(pair.stooq),
    getDXY(),
  ]);

  // Use batch price first (most accurate), fallback to individual sources
  let livePrice = batchPrice || await getStooqPrice(pair.stooq);
  ...
  if(!finalPrice && pair.isGold) {
    try {
      const r = await fetch('https://api.metals.live/v1/spot/gold',{timeout:3000});
...
app.get('/api/signals',async(req,res)=>{
  try{
    ...
    // Return cached signals IMMEDIATELY if available (instant response)
    const cachedSignals=PAIRS.map(p=>cache.signal[p.symbol]?.data).filter(Boolean);
    if(cachedSignals.length===PAIRS.length){
      const cs=calcCurrencyStrength(cachedSignals.map(s=>({symbol:s.symbol,rsi:s.rsi})));
      cachedSignals.forEach(s=>{s.currencyStrength=cs;});
      // Refresh in background for next call
      Promise.allSettled(PAIRS.map(getSignalForPair)).catch(()=>{});
      return res.json({success:true,signals:cachedSignals,count:cachedSignals.length,
        cached:true,blackout:isBlackout(),session:getSession().name,killzone:getSession().killzone,
        marketClosingSoon:isMarketClosingSoon(),dxy:cache.dxy.value,
        time:new Date().toISOString()});
    }

    // If partial cache - return what we have + fetch missing ones
    if(cachedSignals.length>0){
      const missingSyns=PAIRS.filter(p=>!cache.signal[p.symbol]?.data);
      // Add synthetic fallbacks for missing pairs immediately
      for(const pair of missingSyns){
        const fallbackPrice=FALLBACK_PRICES[pair.stooq]||1.0;
        const synCandles=buildSyntheticCandles(fallbackPrice,pair.symbol);
        const mtfBias=getMultiTimeframeBias([],[],[],synCandles);
        const atr=calcATR(synCandles)||0.001;
        const ind={rsi:50,macd:null,emas:null,bollinger:null,stoch:null,adx:null,
          structure:'NEUTRAL',sr:null,candle:'NONE',fib:null,pivots:null,atr,
          fvg:null,bos:null,liquidity:null,orderBlock:null,prevDay:null,
          asiaRange:null,session:getSession(),choch:null,equalHighsLows:null,
          adrCompletion:null,ote:null,judasSwig:null,premiumDiscount:null,
          silverBullet:isSilverBullet(),regime:null,roundNumber:null,
          riskSentiment:null,wyckoff:null,breakerBlock:null,inducement:null,
          powerOfThree:null,realInterestRate:calcRealInterestRate(pair.symbol),
          treasuryYield:null,elliottWave:null,mondayGap:null,
          doubleTopBottom:null,currencyStrength:null};
        const result=deepSignalEngine(ind,mtfBias,cache.dxy.value,pair.symbol);
        const levels=calcLevels(result.signal,fallbackPrice,atr,pair.symbol,null);
        cachedSignals.push({
          symbol:pair.symbol,price:fallbackPrice.toFixed(pair.dp),
          signal:result.signal,phase:result.phase,confidence:result.confidence,
          entry:fallbackPrice.toFixed(pair.dp),sl:levels.sl,tp:levels.tp,rr:levels.rr,
          entryZone:null,bullScore:result.bull,bearScore:result.bear,
          mtfBias:mtfBias.bias,dailyBias:mtfBias.daily,weeklyBias:mtfBias.weekly,
          monthlyBias:mtfBias.monthly,dxy:cache.dxy.value,
          rsi:50,adx:null,structure:'NEUTRAL',candle:'NONE',
          spread:getSpread(pair.symbol),blackout:isBlackout(),
          newsComingSoon:isNewsWithin4H(),marketClosingSoon:isMarketClosingSoon(),
          session:getSession().name,killzone:getSession().killzone,
          dataSource:'Loading...',risk:calcRisk(fallbackPrice,levels.sl,pair.symbol),
          reasons:[{icon:'⏳',text:`Loading ${pair.symbol} data... will update in 30 seconds`}],
          warnings:['⚠️ Using estimated prices - real data loading in background'],
          candles:[],updatedAt:new Date().toISOString(),
          // Force WAIT state for loading signals - don't show wrong confidence
          signal:'WAIT',phase:'ANALYSING',confidence:0,
      ...
      Promise.allSettled(PAIRS.map(getSignalForPair)).catch(()=>{});
      return res.json({success:true,signals:cachedSignals,count:cachedSignals.length,
        loading:true,blackout:isBlackout(),session:getSession().name,
        killzone:getSession().killzone,marketClosingSoon:isMarketClosingSoon(),
        dxy:cache.dxy.value,time:new Date().toISOString()});
    }

    // No cache at all - fetch everything with timeout protection
    const results=await Promise.allSettled(
      PAIRS.map(p=>Promise.race([
        getSignalForPair(p),
...
app.get('/api/news',async(req,res)=>{
  const now=Date.now();
  // Force refresh if cache is older than 5 minutes OR articles are from yesterday
  const cacheAge = now - cache.news.ts;
  const hasStaleNews = cache.news.data.length > 0 &&
    cache.news.data.every(n => {
      const age = now - new Date(n.time).getTime();
      return age > 24*60*60*1000; // older than 24 hours
    });
  if(cache.news.data.length && cacheAge < TTL.NEWS && !hasStaleNews){
...
// Self-ping every 4 minutes to prevent sleeping on free hosts (Glitch etc)
setInterval(()=>{fetch(`http://localhost:${process.env.PORT||3000}/api/health`).catch(()=>{});},4*60*1000);
...
app.listen(PORT,async()=>{
  ...
  // Init NGN rate first
  cachedNGNRate = await getNGNRate();
  console.log(`✅ NGN/USD rate: ${cachedNGNRate}`);
  // Pre-warm batch prices
  console.log('🔥 Pre-warming caches...');
  try{
    await fetchBatchPrices();
    console.log('✅ Batch prices warmed');
    await Promise.allSettled(PAIRS.map(p=>getStooqPrice(p.stooq)));
    console.log('✅ Individual prices warmed');
    setTimeout(async()=>{
